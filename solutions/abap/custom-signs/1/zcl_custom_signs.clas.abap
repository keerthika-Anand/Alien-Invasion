...
CLASS zcl_custom_signs IMPLEMENTATION.
  ...
  METHOD build_birthday_sign.
    DATA(lv_age_word) = COND string( WHEN age >= 50 THEN 'mature' 
                                    ELSE 'young' ).
    result = |Happy Birthday! What a { lv_age_word } fellow you are.|. 
  ENDMETHOD.
  ...
  METHOD cost_of.
    DATA(lv_length) = strlen( sign ).
    DATA(lv_total_cost) = 20 + ( lv_length * 2 ).
    DATA(lv_cost_string) = |{ lv_total_cost DECIMALS = 2 }|.
    result = |Your sign costs { lv_cost_string } { currency }.|.
  ENDMETHOD.
